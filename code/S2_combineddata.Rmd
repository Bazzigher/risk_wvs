...
# Model1 
```{r}
model1_combined <- lmer(risktaking ~ age_scale + gender + 
                        (1 + age_scale + gender | country), 
                        data = merged_data, 
...
# Model2
```{r}
model2_combined <- lmer(risktaking ~ age_scale * hardship_index + 
...
```{r}
countries_per_page <- 105  # Countries per page
...
# Plot all countries combined without density
```{r}
pdf("combined_all_countries_no_density.pdf", width = 10, height = 8)  # Adjust width and height as needed

# Plot scatter without density
plot(merged_data$age, merged_data$risktaking, type = "n", xlab = "Age", ylab = "Risk-Taking Propensity", 
     xlim = c(min(merged_data$age, na.rm = TRUE), max(merged_data$age, na.rm = TRUE)), 
     ylim = c(min(merged_data$risktaking, na.rm = TRUE), max(merged_data$risktaking, na.rm = TRUE)), 
     main = "All Countries")

# Add scatter points
points(merged_data$age, merged_data$risktaking, pch = 20, col = "white")
...
# Plot individual countries without density
```{r}
# Schleife über alle Seiten
for (page in 1:num_pages) {
  # PDF-Datei für die aktuelle Seite erstellen
  pdf(paste0("combined_individual_country_no_density", page, ".pdf"), width = 10, height = 8)
  ...
  for (country in countries_on_page) {
    ...
    # Plot für das aktuelle Land erstellen
    plot(data_country$age, data_country$risktaking, type = "n", xlab = "Age", ylab = "Risk-Taking Propensity", 
         xlim = c(min(data_country$age, na.rm = TRUE), max(data_country$age, na.rm = TRUE)), 
         ylim = c(min(data_country$risktaking, na.rm = TRUE), max(data_country$risktaking, na.rm = TRUE)), 
         main = country)
    
    # Add scatter points
    points(data_country$age, data_country$risktaking, pch = 20, col = "white")
    ...
    # Plot regression lines für das aktuelle Land plotten
    lines(age_range, predictions_female, col = "blue", lty = 1, lwd = 3)
    lines(age_range, predictions_male, col = "green", lty = 2, lwd = 3)
  }
