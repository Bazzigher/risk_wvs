...
```
```{r}
# Checking Log Transformation and Scaling/Reversing:

summary(hardship_complete[, c("homiciderate", "gdp", "infantmortality", "lifeexpectancy")])
# Visualize distributions before and after transformation
ggplot(hardship_complete, aes(x = gdp)) + 
  geom_histogram(bins = 20, fill = "lightblue", color = "black") +
  ggtitle("Distribution of GDP Before Transformation") +
  xlab("GDP") + ylab("Frequency")

ggplot(hardship_complete, aes(x = scale(-gdp))) + 
  geom_histogram(bins = 20, fill = "lightblue", color = "black") +
  ggtitle("Distribution of GDP After Scaling/Reversing") +
  xlab("Scaled/Reversed GDP") + ylab("Frequency")
```

# Checking Effectiveness of Imputation Method
```{r}
summary(hardship_complete[, c("homiciderate", "gdp", "infantmortality", "lifeexpectancy", "gini", "femalemale_primedu")])
```
...
options(scipen = 100)
model_2 <- lmer(T_score_risktaking ~ 1 + scale(age) + factor(gender) + factor(children) + factor(married) + factor(employed) + factor(education) + (1 + scale(age) + factor(gender) + factor(children) + factor(married) + factor(employed) + factor(education) | country),
...
model_3 <- lmer(T_score_risktaking ~ 1 + scale(age) * hardship_index + 
                    factor(gender) * hardship_index + factor(children) + factor(married) + factor(employed) + factor(education) + 
                    (1 + scale(age) + factor(gender)+ factor(children) + factor(married) + factor(employed) + factor(education) | country),
