...
```{r}
# Define the filename for the PDF
pdf("fixed_effects_plot_all_countries_Laura.pdf", width = 10, height = 8)  # Adjust width and height as needed
...
# Plots all countries combined datasets (WVS, GPS)
```{r}
# Define the filename for the PDF
pdf("combined_data_combined_countries.pdf", width = 10, height = 8)  # Adjust width and height as needed
...
# Plots country individually with combined datasets (WVS, GPS)
```{r}
...
# Define the filename for the PDF
pdf("combined_data_by_country_individual.pdf", width = 10, height = 8)

# Define the layout for the plots: 2x2
par(mfrow = c(2, 2))
...
for (i in 1:n_countries) {
  ...
  # Plot regression lines
  curve(regline_age(x), from = min(country_data$age, na.rm = TRUE), to = max(country_data$age, na.rm = TRUE), add = TRUE, col = "blue", lty = 1, lwd = 3)
  curve(regline_gender(x), from = min(country_data$age, na.rm = TRUE), to = max(country_data$age, na.rm = TRUE), add = TRUE, col = "green", lty = 2, lwd = 3)

  # When four plots are on one page, start a new page
  if (i %% 4 == 0 && i != n_countries) {
    par(mfrow = c(2, 2))  # Reset layout for new page
  }
}
...
# Plots all countries GPS
```{r}
# Define the filename for the PDF
pdf("gps_country_plot.pdf", width = 10, height = 8)  # Adjust width and height as needed
...
# Plots country individually GPS
```{r}
# Load necessary package
library(dplyr)

# Create a list of unique countries
countries <- unique(gps_data$country)
n_countries <- length(countries)

# Define the filename for the PDF
pdf("gps_country_plot_individual.pdf", width = 10, height = 8)
...
# Plots all countries WVS
```{r}
# Define the filename for the PDF
pdf("wvs_country.pdf", width = 10, height = 8)  # Adjust width and height as needed
...
# Plots country individually WVS
```{r}
# Load necessary package
library(dplyr)

# Create a list of unique countries
countries <- unique(mixed_model_data$country)
n_countries <- length(countries)

# Define the filename for the PDF
pdf("wvs_country_individueal.pdf", width = 10, height = 8)
