...
# Input Laur: This code gave me an error
...
```{r}
# Zuweisung der 'code'-Spalte von countryfacts zur neuen Spalte 'codeWVS', falls noch nicht vorhanden
countryfacts$codeWVS <- countryfacts$code

# Erstellen der modelcoefs Daten
modelcoefs <- ranef(model2_wvs)$country
modelcoefs$codeWVS <- row.names(modelcoefs)
...
# Plot fixed for all countries // New with density
```{r}
# Define the filename for the PDF
...
# Erstellen der modelcoefs Daten
modelcoefs <- ranef(model2_gps)$country
modelcoefs$codeWVS <- row.names(modelcoefs)
...
# Model1 (Achtung: es war folgendes drin vorher "data = gps_data", ich habe das nun angepasst)
```{r}
...
# Plot all countries combined with tidyverse
```{r}
library(ggplot2)

# Umwandeln der Geschlechtsvariable in einen Faktor, falls nicht bereits geschehen
merged_data$gender <- factor(merged_data$gender, levels = c("male", "female"))

# Erstellen des ggplot2-Objekts
my_plot <- ggplot(merged_data, aes(x = age, y = risktaking, color = gender, group = gender)) + 
  geom_point(alpha = 0.5) +  # Punkte halbtransparent darstellen
  scale_color_manual(values = c("male" = "blue", "female" = "red")) +  # Farben für die Geschlechter definieren
  geom_smooth(method = "lm", linetype = "dashed", se = FALSE) +  # Regressionslinien für jedes Geschlecht
  theme_minimal() +
  labs(x = "Age", y = "Risk-Taking Propensity (T score)", color = "Gender") +
  theme(legend.position = "top") +
  scale_y_continuous(limits = c(30, NA))  # Untergrenze für y-Achse setzen

# Anzeige des Plots
print(my_plot)

# Speichern des Plots als PDF
ggsave("plot_GPS_tidy.pdf", my_plot, width = 10, height = 8, units = "in", dpi = 300)
```
