...
# E_exp_firearms
```{r}
# Sicherstellen, dass die Spalte firearms numerisch ist und sie umbenennen
E_exp_firearms <- firearms %>%
  mutate(firearms = as.numeric(firearms)) %>%  # Konvertieren von firearms in numerisch
  rename(E_exp_firearms = firearms)           # Umbenennen in E_exp_firearms

# Die Spalte "code" als "COUNTRY" verwenden
E_exp_firearms <- E_exp_firearms %>%
  rename(COUNTRY = code)
...
hardship_environment <- left_join(hardship_environment, E_exp_firearms[, c("COUNTRY", "E_exp_firearms")], by = "COUNTRY")
...
selected_vars <- hardship_environment %>%
  ...
  select(avg_risktaking, starts_with("E_")) %>%  # Wähle relevante Spalten
...
selected_vars <- hardship_environment %>%
  ungroup() %>%  # Gruppierung entfernen
  select(avg_risktaking, E_oth_safewater, E_exp_watersanithyg100k, E_exp_airdeath100k, E_ses_gini, E_ses_unemployment, E_ses_school, E_exp_firearms, E_ses_water) %>%  # Auswahl der Variablen
  drop_na()  # Entferne Zeilen mit fehlenden Werten

# 2. Modell anpassen
model <- lm(avg_risktaking ~ E_oth_safewater + E_exp_watersanithyg100k + E_exp_airdeath100k + E_ses_gini + E_ses_unemployment + E_ses_school + E_exp_firearms, E_ses_water, data = selected_vars)
...
sensitivity <- sensemakr(
  model = model,
  treatment = "E_oth_safewater", # Schlüsselvariable, deren Effekt getestet wird
  benchmark_covariates = c("E_exp_watersanithyg100k", "E_exp_airdeath100k"), # Störvariablen
...
methods["E_exp_firearms"] <- "pmm"           # Numerisch
```
...
imputed_data$imp$E_exp_firearms  # Zeigt die imputierten Werte für diese Variable
...
hardship_environment <- hardship_environment %>%
  mutate(
    ...
    E_exp_firearms = completed_data$E_exp_firearms
  )
