...
# Umbenennen per rename_with (sauberer als sapply/ifelse)
library(dplyr)

# Neue Version des Datensatzes mit Klartextnamen nur für 'e_'-Variablen
hardship_environment_labeled <- hardship_environment %>%
  rename_with(~ environment_factor_labels[.x], .cols = names(environment_factor_labels))

```


```{r}
# vis_miss mit Klarnamen
vis_miss(hardship_environment_labeled) +
...
# aggr mit Klarnamen
aggr(
  hardship_environment_labeled,
  col = c('navyblue', 'red'),
  numbers = TRUE,
  sortVars = TRUE,
  labels = names(hardship_environment_labeled),
  cex.axis = 0.7,
  gap = 3,
...
# Berechnung des Anteils fehlender Werte pro Variable
...
# Neue Spalte mit Klarnamen (falls vorhanden)
missing_summary$Label <- ifelse(
  missing_summary$Variable %in% names(environment_factor_labels),
  environment_factor_labels[missing_summary$Variable],
  NA  # NA, wenn kein Mapping vorhanden ist
)

# Sortiere nach Anteil fehlender Werte
...
stripplot(imputed_data,
          e_exp_watersanithyg100k + e_ses_gini + e_exp_disaster +
          e_ses_school + e_exp_airdeath100k + e_exp_watersanithyg ~ .imp,
          pch = 20, cex = 1.2,
          scales = list(y = list(relation = "free")))

densityplot(imputed_data,
            ~ e_oth_safewater + e_exp_watersanithyg100k + e_exp_disaster +
              e_exp_airdeath100k + e_ses_gini + e_ses_school,
...
# Mapping für technische Variablennamen zu Klarnamen
environment_factor_labels <- c(
  e_oth_drinkingwater       = "Access to Drinking Water",
  e_exp_watersanithyg100k   = "WASH-related Deaths (per 100k)",
  e_exp_watersanithyg       = "Total WASH-Related Diarrhoea Deaths",
...
selected_vars <- hardship_environment %>%
  select(all_of(names(environment_factor_labels))) %>%
  select(where(~ !(all(is.na(.)) | sd(., na.rm = TRUE) == 0)))  # Entfernt leere oder konstante Spalten

# Korrelationsmatrix
cor_matrix <- cor(selected_vars, use = "complete.obs")

# Melt und Klartextnamen zuweisen
melted_cor_matrix <- melt(cor_matrix)
melted_cor_matrix$Var1 <- environment_factor_labels[as.character(melted_cor_matrix$Var1)]
melted_cor_matrix$Var2 <- environment_factor_labels[as.character(melted_cor_matrix$Var2)]
