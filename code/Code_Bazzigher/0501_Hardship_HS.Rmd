...
mcar_test <- mcar_test(hardship_HS)
...
# Cronbach's Alpha
```{r}
# Reverse and convert specified variables to numeric
hardship_HS <- hardship_HS %>%
  mutate(
    HS_alc_agelim = as.numeric(max(HS_alc_agelim, na.rm = TRUE) + 1 - HS_alc_agelim),
    HS_sex_antiretroviral = as.numeric(max(HS_sex_antiretroviral, na.rm = TRUE) + 1 - HS_sex_antiretroviral),
    HS_oth_cleancooking = as.numeric(max(HS_oth_cleancooking, na.rm = TRUE) + 1 - HS_oth_cleancooking),
    HS_mh_mhhospit = as.numeric(max(HS_mh_mhhospit, na.rm = TRUE) + 1 - HS_mh_mhhospit),
    HS_mh_policy = as.numeric(max(HS_mh_policy, na.rm = TRUE) + 1 - HS_mh_policy),
    HS_oth_obesity = as.numeric(max(HS_oth_obesity, na.rm = TRUE) + 1 - HS_oth_obesity)
  )

# Select only numeric variables for PCA
numeric_vars_for_pca <- select_if(hardship_HS, is.numeric)

# Confirming that only numeric variables are being considered
if (!all(sapply(numeric_vars_for_pca, is.numeric))) {
  stop("Non-numeric columns present in the dataset used for PCA")
}

# Load the necessary library
library(psych)

# Conduct PCA on the numeric variables only
pca_results <- principal(numeric_vars_for_pca, nfactors = 5, rotate = "varimax")

# Print the loadings to check the direction
print(pca_results$loadings)

# Check summary of PCA results
summary(pca_results)

# Print eigenvalues directly
print(pca_results$values)
# Load ggplot2 for plotting
library(ggplot2)

# Create a data frame for the eigenvalues
eigen_df <- data.frame(
...
ggplot(eigen_df, aes(x = Component, y = Eigenvalues)) +
  geom_line() +
  geom_point() +
  theme_minimal() +
  labs(title = "Scree Plot", x = "Component Number", y = "Eigenvalues") +
  geom_hline(yintercept = 1, linetype = "dashed", color = "red")  # Cutoff for eigenvalues > 1
```
