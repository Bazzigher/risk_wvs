---
title: "4_Hardship_specification_curve_Health_Safety"
output: 
  html_document:
    code_folding: hide
date: "2024-05-08"
...
HS_sex_antiretroviral <- HS_sex_antiretroviral %>% select(IndicatorCode, COUNTRY, Year_2005, Year_2006, Year_2007, Year_2008, Year_2009, Year_2010, Year_2011, Year_2012, Year_2013, Year_2014)
...
manual_mh_policy <- tibble(
  COUNTRY = c("COL", "URY", "LBY", "GBR", "VEN", "PSE", "TWN"), 
  MH_policy_manual = c("Yes", "Yes", "Yes", "Yes", "Yes", "Yes", "Yes") # Verwenden eines eindeutigen Namens für die manuelle Spalte
)
...
HS_mh_policy <- HS_mh_policy %>%
  ...
  mutate(
    HS_mh_policy = coalesce(MH_policy_manual, HS_mh_policy) # Bevorzugt die manuellen Werte
  ) %>%
  select(COUNTRY, HS_mh_policy) # Überflüssige Spalten entfernen

# Transformation der "Yes"/"No" Werte in numerische Codes
HS_mh_policy <- HS_mh_policy %>%
  mutate(
    HS_mh_policy = case_when(
      HS_mh_policy == "Yes" ~ 0,  # Niedriger Wert für vorhandene Politik
      HS_mh_policy == "No" ~ 1,   # Hoher Wert für fehlende Politik
      TRUE ~ as.numeric(HS_mh_policy) # Konvertiert Zeichenketten, die numerische Werte sind, in numerische Typen
    )
  )
...
missing_countries <- grep("^HS_", names(hardship_HS), value = TRUE)

# Berechnung des Prozentsatzes der fehlenden Werte pro Land
missing_percentages <- hardship_HS %>%
  select(country, all_of(missing_countries)) %>%
  group_by(country) %>%
  summarise_each(funs(sum(is.na(.)) / n() * 100), all_of(missing_countries)) %>%
  mutate(average_missing = rowMeans(select(., -country), na.rm = TRUE))  # Berechnet den Durchschnitt der fehlenden Werte über alle "F_" Variablen
...
# Klartextnamen hinzufügen
missing_summary <- missing_summary %>%
  left_join(risk_factor_df, by = c("Variable" = "risk_factor")) %>%
...
## Little's MCAR-Test 
```{r}
# Durchführung von Little's MCAR-Test mit dem 'naniar' Paket
if (!require("naniar")) install.packages("naniar")
library(naniar)

# Durchführen des Tests
mcar_results <- mcar_test(hardship_HS)
...
# Spaltennamen durch Klartextnamen ersetzen (nur für Visualisierung)
hardship_HS_labeled <- hardship_HS
names(hardship_HS_labeled) <- ifelse(
  names(hardship_HS_labeled) %in% names(risk_factor_labels),
  risk_factor_labels[names(hardship_HS_labeled)],
  names(hardship_HS_labeled)
)

par(mar = c(5, 4, 4, 10))  # <- das ist der entscheidende Teil!
...
# Aggregierter Missingness-Plot
aggr_plot <- aggr(hardship_HS_labeled,
                  col = c('navyblue', 'red'),
                  numbers = TRUE,
                  sortVars = TRUE,
                  labels = short_labels,
...
# Plot 2: naniar::vis_miss()
vis_miss(hardship_HS_labeled) +
...
methods["HS_original_infantmortality"] <- "pmm"             # Numerisch
methods["HS_original_lifeexpectancy"] <- "pmm"                      # Numerisch
methods["HS_original_genderequality"] <- "pmm"           # Numerisch
...
imputed_data <- mice(
  hardship_HS_numeric,
  m = 20,            # 20 vollständige Datensätze erzeugen
...
stripplot(imputed_data,
          HS_sex_antiretroviral + HS_drg_treatment + HS_alc_roaddeath + HS_mh_mhhospit +
          HS_sex_gini ~ .imp,
          pch = 20, cex = 1.2,
...
hardship_HS <- hardship_HS %>%
  mutate(
    HS_original_genderequality = completed_data$HS_original_genderequality, 
    HS_original_infantmortality = completed_data$HS_original_infantmortality, 
    HS_original_lifeexpectancy = completed_data$HS_original_lifeexpectancy, 
...
hardship_HS <- hardship_HS %>%
  ...
  rowwise() %>%
  mutate(
    hardship_HS_index = mean(c(HS_original_infantmortality, HS_original_genderequality, HS_original_lifeexpectancy,HS_alc_agelim, HS_sex_antiretroviral, HS_oth_cleancooking, HS_mh_mhhospit, HS_alc_tax_wine, HS_alc_roaddeath, HS_nic_affordability, HS_sex_gini, HS_drg_treatment, HS_mh_policy, HS_oth_obesity), na.rm = TRUE)
  ) %>%
  ungroup()
...
```{r}
hardship_HS <- hardship_HS %>%
  select(-HS_alc_agelim, -HS_original_infantmortality)
```
...
cor_matrix <- cor(hardship_HS %>% select_if(is.numeric), use = "complete.obs")
...
selected_vars <- hardship_HS %>% 
  select(avg_risktaking, starts_with("HS_"))
...
# Mapping der technischen Variablennamen auf Klarnamen
hs_labels <- c(
  HS_alc_tax_wine = "Alcohol Tax (Wine)",
  HS_alc_roaddeath = "Alcohol & Road Deaths",
  HS_drg_treatment = "Drug Treatment Availability",
...
selected_vars <- hardship_HS %>% 
  select(all_of(names(hs_labels)))  # wichtig: nur die gemappten Variablen

# Berechne Korrelationsmatrix (mit benannten Spalten/Zeilen)
cor_matrix <- cor(selected_vars, use = "complete.obs")

# Umwandeln in langen Datensatz für ggplot
melted_cor_matrix <- melt(cor_matrix)

# Ersetze technische Namen durch Klarnamen
melted_cor_matrix$Var1 <- hs_labels[as.character(melted_cor_matrix$Var1)]
melted_cor_matrix$Var2 <- hs_labels[as.character(melted_cor_matrix$Var2)]
...
ggplot(melted_cor_matrix, aes(Var1, Var2, fill = value)) +
  ...
  scale_fill_gradient2(
    midpoint = 0, low = "blue", high = "red", mid = "white", limits = c(-1, 1)
  ) +
  theme_minimal() +
  theme(
...
items <- hardship_HS %>%
  select(HS_original_genderequality, HS_original_lifeexpectancy, 
         HS_alc_tax_wine, HS_alc_roaddeath, HS_drg_treatment, 
