...
## Risktaking 
```{r}
library(ggrepel)  # <- vorher laden!

ggplot(new_data, aes(x = intercept_gps, y = intercept_WVS)) +
  geom_point(size = 1.5) +
  geom_text_repel(aes(label = code), size = 3, max.overlaps = Inf) +
  geom_smooth(method = "lm", se = FALSE, linetype = "dashed") +
  annotate("text", 
           x = min(new_data$intercept_gps, na.rm = TRUE),
           y = max(new_data$intercept_WVS, na.rm = TRUE),
           label = "(A)", fontface = "bold", size = 6, hjust = -0.2, vjust = 1) +
  labs(x = "Intercept GPS", y = "Intercept WVS") +
  theme_classic() +
  coord_fixed()
...
## Age
```{r}
ggplot(new_data, aes(x = slope_age_gps, y = slope_age_WVS)) +
  geom_point(size = 1.5) +
  geom_text_repel(aes(label = code), size = 3, max.overlaps = Inf) +
  geom_smooth(method = "lm", se = FALSE, linetype = "dashed") +
  annotate("text", 
           x = min(new_data$slope_age_gps, na.rm = TRUE),
           y = max(new_data$slope_age_WVS, na.rm = TRUE),
           label = "(B)", fontface = "bold", size = 6, hjust = -0.2, vjust = 1) +
  labs(x = "Age Slope GPS", y = "Age Slope WVS") +
  theme_classic() +
  coord_fixed()
...
## Gender
```{r}
ggplot(new_data, aes(x = slope_gender_gps, y = slope_gender_WVS)) +
  geom_point(size = 1.5) +
  geom_text_repel(aes(label = code), size = 3, max.overlaps = Inf) +
  geom_smooth(method = "lm", se = FALSE, linetype = "dashed") +
  annotate("text", 
           x = min(new_data$slope_gender_gps, na.rm = TRUE),
           y = max(new_data$slope_gender_WVS, na.rm = TRUE),
           label = "(C)", fontface = "bold", size = 6, hjust = -0.2, vjust = 1) +
  labs(x = "Gender Slope GPS", y = "Gender Slope WVS") +
  theme_classic() +
  coord_fixed()
...
```{r}
cor_intercept <- cor(new_data$intercept_gps, new_data$intercept_WVS, use = "complete.obs")
cor_age <- cor(new_data$slope_age_gps, new_data$slope_age_WVS, use = "complete.obs")
cor_gender <- cor(new_data$slope_gender_gps, new_data$slope_gender_WVS, use = "complete.obs")
