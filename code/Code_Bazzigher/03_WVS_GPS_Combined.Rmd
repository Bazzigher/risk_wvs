...
for (i in seq(1, length(countries), by = num_countries_per_page)) {
  ...
  filename <- paste0("plots/Individual_countries_model1_page_", ceiling(i / num_countries_per_page), ".png")
...
iso_codes <- merged_data %>% select(country, isocode) %>%
             distinct()
analysis_df <- merge(analysis_df, iso_codes, by = "country", all.x = TRUE)

# Plot erstellen
p <- ggplot(analysis_df, aes(x = median_hardship, y = gender, label = isocode)) +
  geom_text(aes(label = isocode), check_overlap = TRUE, size = 3, vjust = 1) +
  geom_smooth(method = "lm", color = "black", se = FALSE) +  # Regressionslinie hinzufügen
  labs(title = "Gender Effect vs. Hardship Index\nR² = 0.17",
       x = "Hardship Index",
       y = "Gender Effect") +
...
# Plot für Age Effect erstellen
p_age <- ggplot(analysis_df, aes(x = median_hardship, y = age, label = isocode)) +
  geom_text(aes(label = isocode), check_overlap = TRUE, size = 3, vjust = 1) +
  geom_smooth(method = "lm", color = "black", se = FALSE) +  # Regressionslinie hinzufügen
  labs(title = "Age Effect vs. Hardship Indexn
       R² = 0.32",
       x = "Hardship Index",
       y = "Age Effect") +
  theme_minimal(base_size = 14) +
...
# Intercept of Risktaking Plot anpassen
p_intercept <- ggplot(analysis_df, aes(x = median_hardship, y = `(Intercept)`, label = isocode)) +
  geom_text(aes(label = isocode), check_overlap = TRUE, size = 3, vjust = 1) +
  geom_smooth(method = "lm", color = "black", se = FALSE) +
  scale_x_continuous(limits = c(-3, 4)) +  # Anpassung der X-Achsen-Grenzen
scale_y_continuous(limits = c(-2, 4.5)) +
  labs(title = "Intercept of Risktaking vs. Hardship Index\nR² = 0.03",
       x = "Hardship Index",
       y = "Intercept of Risktaking") +
  theme_minimal() +
  coord_fixed(ratio = 1)
