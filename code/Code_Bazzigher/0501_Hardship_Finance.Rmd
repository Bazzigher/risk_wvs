...
# Daten abrufen
control <- WDI(country = countries, indicator = indicators, start = 2005, end = 2014)
...
```{r}
hardship_Finance <- hardship_Finance %>% select(COUNTRY, country, avg_risktaking, homiciderate, gdp, infantmortality, lifeexpectancy, gini, femalemale_primedu, f_inv_acctownership_primaryedu,  f_oth_insfinsvcs_int, f_inv_bankfin_invest, f_inv_bankfin_workcap, f_hs_oopexp10, f_eco_gdpdefl_linked, f_hs_oopexp_pov215, f_eco_cpi)
str(hardship_Finance)
```
...
mcar_test <- mcar_test(hardship_Finance)
...
# Plot Missingness 
# Beispiel: Missingness von f_hs_oopexp10 in Bezug auf avg_risktaking
ggplot(hardship_Finance, aes(x = avg_risktaking, y = is.na(f_hs_oopexp10))) +
  geom_jitter() +
  labs(title = "Missingness in f_hs_oopexp10 vs avg_risktaking")
...
#hardship_Finance <- hardship_Finance %>% select(-f_hs_oopexp10)
...
```{r}
sapply(hardship_Finance, function(x) all(x == x[1], na.rm = TRUE))

cor_matrix <- cor(hardship_Finance, use = "pairwise.complete.obs")
high_cor <- which(abs(cor_matrix) > 0.99 & abs(cor_matrix) < 1, arr.ind = TRUE)
print(high_cor)

sum(complete.cases(hardship_Finance))
...
mean_values_pre_imputation <- sapply(hardship_Finance, function(x) mean(x, na.rm = TRUE))
...
hardship_Finance <- hardship_Finance %>%
  mutate(
    ...
    f_hs_oopexp10 = scale(f_hs_oopexp10), # keine Umkehrung
    f_eco_gdpdefl_linked = scale(f_eco_gdpdefl_linked), # keine Umkehrung
    f_eco_cpi = scale(f_eco_cpi) # keine Umkehrung
  ) %>%
  rowwise() %>%
  mutate(
    hardship_Finance_index = mean(c(f_inv_acctownership_primaryedu, f_oth_insfinsvcs_int, f_hs_oopexp10, f_eco_gdpdefl_linked, f_eco_cpi), na.rm = TRUE)
  ) %>%
  ungroup()
