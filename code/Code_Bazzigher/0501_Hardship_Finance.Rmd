...
# Nur die relevanten Variablen extrahieren
finance_vars <- names(finance_factor_labels)
hardship_finance_subset <- hardship_Finance[, finance_vars]

# Fehlende Werte berechnen
missing_percentages <- sapply(hardship_finance_subset, function(x) {
...
missing_summary <- data.frame(
  ...
  MissingPercentage = round(missing_percentages, 2),
  Label = finance_factor_labels[names(missing_percentages)]
)

# Nach Missing Percentage sortieren
...
# Subset mit nur den relevanten Variablen
finance_vars <- names(finance_factor_labels)
hardship_finance_subset <- hardship_Finance[, finance_vars]

# Klartextnamen als Spaltennamen setzen
colnames(hardship_finance_subset) <- finance_factor_labels[finance_vars]

# VIM::aggr Plot mit Klartextnamen
short_labels <- c(
  "Acct.Ownership",
  "Fin. Imports",
  "Out-of-Pocket",
...
vis_miss(hardship_finance_subset) +
...
stripplot(imputed_data,
          f_hs_oopexp10 + f_oth_insfinsvcs_int + f_inv_acctownership_primaryedu + f_eco_cpi
          ~ .imp,
          pch = 20, cex = 1.2,
...
## Corrlation Heatmap with f_-variables
```{r}
...
# Klartextnamen für Finance Hardship Indicators (ohne avg_risktaking)
finance_factor_labels <- c(
  f_inv_acctownership_primaryedu = "Financial Account Ownership (Low Edu)",
  f_oth_insfinsvcs_int = "Insurance & Financial Service Imports",
  f_hs_oopexp10 = "High Out-of-Pocket Health Spending",
...
# Auswahl relevanter f_-Variablen (ohne avg_risktaking)
selected_vars <- hardship_Finance %>%
  select(all_of(names(finance_factor_labels))) %>%
  select(where(~ !(all(is.na(.)) | sd(., na.rm = TRUE) == 0)))  # entfernt konstante oder leere Spalten

# Berechnung der Korrelationsmatrix
cor_matrix <- cor(selected_vars, use = "complete.obs")

# Umwandlung für ggplot
melted_cor_matrix <- melt(cor_matrix)

# Klartextnamen ersetzen
replace_with_label <- function(x) {
  ifelse(x %in% names(finance_factor_labels), finance_factor_labels[x], x)
}

melted_cor_matrix$Var1 <- replace_with_label(melted_cor_matrix$Var1)
melted_cor_matrix$Var2 <- replace_with_label(melted_cor_matrix$Var2)
