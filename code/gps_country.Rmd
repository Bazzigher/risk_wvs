...
# Risk vs age with color-coded gender
```{r}
...
# GDP vs risktaking with color-coded gender and customized legend labels
ggplot(cleaned_data, aes(x = ranking, y = risktaking, color = factor(gender))) +
  geom_point(size = 0.2) +  
  geom_smooth(method = "lm") +
  scale_color_manual(values = c("blue", "red"), labels = c("Male", "Female")) +
  labs(x = "GDP", y = "Risktaking", color = "Gender")
...
# Create data frames for different country statuses
only_in_cleaned_data <- setdiff(cleaned_data$country, data$country_lab)
only_in_data <- setdiff(data$country_lab, cleaned_data$country)
common_countries <- intersect(cleaned_data$country, data$country_lab)

cleaned_data_map <- data.frame(country = only_in_cleaned_data, status = "Only in cleaned_data")
data_map <- data.frame(country = only_in_data, status = "Only in data")
common_countries_map <- data.frame(country = common_countries, status = "Common")

# Combine the data frames
world_map_data <- rbind(cleaned_data_map, data_map, common_countries_map)

# Merge with world map data
world_map_data <- merge(world_map, world_map_data, by.x = "name", by.y = "country", all.x = TRUE)

# Create the world map with the desired colors
ggplot() +
  geom_sf(data = world_map_data, aes(fill = status), color = "white") +
  scale_fill_manual(values = c("Only in cleaned_data" = "green", "Only in data" = "blue", "Common" = "pink")) +
  labs(title = "Country Status on World Map", fill = "Status") +
  theme_void() +
  theme(legend.position = "bottom")
