...
```
```{r}
# Installiere und lade die openxlsx-Bibliothek
...
setwd("/Users/laurabazzigher/Documents/GitHub/risk_wvs/data/dataset/Hardship")

# Lese die Excel-Datei ein
data <- read.xlsx("Hardship_complete.xlsx", sheet = 1)

# Funktion zur Berechnung des Z-Scores
calculate_zscore <- function(x) {
  if(is.numeric(x)) {
    return((x - mean(x, na.rm = TRUE)) / sd(x, na.rm = TRUE))
  } else {
    return(x)  # Lasse nicht-numerische Werte unverändert
  }
}

# Wende die Funktion auf jede numerische Spalte an (außer der "country"-Spalte)
z_scores <- as.data.frame(lapply(data[, -1], calculate_zscore))

# Füge die "country"-Spalte zum Ergebnis hinzu
z_scores <- cbind(data$country, z_scores)
...
# Füge die Durchschnittsspalte "hardship" hinzu
z_scores$hardship <- rowMeans(data[, c("mean_homicide", "gdp", "gini_income", "Infant_mortality", "life_expect")], na.rm = TRUE)

# Füge eine neue Zeile mit dem Durchschnitt ("hardship") für jedes Land hinzu
average_row <- colMeans(z_scores[, sapply(z_scores, is.numeric)], na.rm = TRUE)
average_row <- c("Average", average_row, mean(data$hardship, na.rm = TRUE))
z_scores <- rbind(z_scores, average_row)
...
print(colnames(z_scores)) # Display column names in hardship_data

# Füge die "hardship"-Spalte aus der hardship_values-Tabelle zu cleaned_data hinzu
cleaned_data <- left_join(cleaned_data, select(z_scores, country, hardship), by = "country")
...
ggplot(gps_country, aes(x = risktaking, y = hardship, label = isocode)) +
  geom_point(size = 3) +
  geom_text_repel(
    aes(label = isocode),
    box.padding = 0.5,
  ...
  labs(title = "Risk taking and hardship",
       x = "Risk Taking",
       y = "Hardship") +
  theme_minimal() +
  xlim(c(0, 1.2)) +
...
wordcloud(words = gps_country$isocode, freq = rep(1, nrow(gps_country)), scale = c(2, 0.5))
```
