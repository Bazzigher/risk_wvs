...
merged_data <- merge(cleaned_data, gdp_data, by.x = "country", by.y = "name", all.x = TRUE)
...
```{r}
cleaned_data <- cleaned_data %>%
  select(country, isocode, ison, risktaking, gender, age, z_score_age, agecat, value.x, date_of_information.x, ranking.x)
cleaned_data
```
...
# Scatterplot für risktaking in Abhängigkeit von Alter (age) und Geschlecht (gender)
```{r}
# Scatterplot für Risktaking in Abhängigkeit von Alter (Age)
ggplot(cleaned_data, aes(x = age, y = risktaking)) +
  geom_point(aes(color = gender)) +
  labs(title = "Scatterplot of Risktaking by Age", x = "Age", y = "Risktaking", color = "Gender") +
  theme_minimal()
```

```{r}
# Scatterplot für Risktaking in Abhängigkeit von Alter (Age) mit separaten Loess-Linien für Männer und Frauen
ggplot(cleaned_data, aes(x = age, y = risktaking, color = gender)) +
  geom_point() +
  geom_smooth(method = "loess", se = FALSE, size = 1, aes(group = gender), 
              linetype = "solid", color = "red", show.legend = FALSE) +  # Loess-Linie für Männer
  geom_smooth(method = "loess", se = FALSE, size = 1, aes(group = gender), 
              linetype = "solid", color = "green", show.legend = FALSE) +  # Loess-Linie für Frauen
  labs(title = "Scatterplot of Risktaking by Age with Loess Lines for Gender", x = "Age", y = "Risktaking", color = "Gender") +
  theme_minimal()
...
length(unique(combined_data$country_lab))
```
```{r}
